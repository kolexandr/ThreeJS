...
let bool_krab = null;
let action_krab = null;

let winter_hat = new THREE.Group();
...
function createDirectionalLight(){
	...
	dirLight.shadow.camera.far = 350;   //50   	 default
	dirLight.shadow.camera.left = -270; //-20
	dirLight.shadow.camera.top = 270;
	dirLight.shadow.camera.right = 270;
	dirLight.shadow.camera.bottom = -270;
...
function createRoof(){
	...
	texture.colorSpace= THREE.SRGBColorSpace;
	const material= new THREE.SpriteMaterial({map : texture, color: 0x0468bf});
	...
	createKrab(){
		...
		loader.load('./models/mr_krabs/scene.gltf',
			function (gltf){
				...
				mixers.push(mixer);
				action_krab = mixer.clipAction(animation);
...
class JellyFish{
	...
	createJelly_Fish(posx, posz){
		let jelly = new THREE.Group();
		const loader = new GLTFLoader();

		const onLoad = (gltf, grp) => {
			const model = gltf.scene;
		...
		loader.load('./models/jelly_fish/scene.gltf', gltf => onLoad(gltf, jelly), onProgress, onError);
		jelly.position.set(posx, 20, posz);
		jelly.scale.set(0.05, 0.05, 0.05);

		this.numofJelly++;
		jelly.name = "jelly-" + this.numofJelly;
		scene.add(jelly);
	}

	addJelly(){
		let jellyPosX =   Math.round((Math.random() * 300));
		let jellyPosZ =   Math.round((Math.random() * 250));
		this.createJelly_Fish(jellyPosX, jellyPosZ);
	}

	removeJelly(){
		if (this.numofJelly > 0){
			let lastJelly = scene.getObjectByName("jelly-" + this.numofJelly);
			scene.remove(lastJelly);
			this.numofJelly--;
		}
	}
}
...
function createChristmas_hat(){
	...
	loader.load('./models/christmas_hat/scene.gltf',
		function (gltf){
			...
			model.scale.set(85, 50, 50);


			winter_hat.position.set(0, -25, 0);
			scene.add(winter_hat);

			winter_hat.add(model);
			winter_hat.name="winter_hat";
		},
		function(xhr){
			console.log((xhr.loaded / xhr.total * 100) + '% loaded');
		},
